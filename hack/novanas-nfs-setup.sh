...
# NOTE: NOVANAS_PASS is interpolated into a heredoc by the LOCAL shell. Passwords
# containing $, `, or \ will be mangled. Quote / escape such characters out-of-band
# if your password contains them.

if ! command -v sshpass >/dev/null 2>&1; then
  echo "ERROR: sshpass not on PATH. Install it (brew install hudochenkov/sshpass/sshpass) and re-run." >&2
  exit 1
fi

echo "==> Configuring NFS server on $NOVANAS_HOST..."
...
if ! grep -qE "^${EXPORT_DIR}[[:space:]]" /etc/exports 2>/dev/null; then
